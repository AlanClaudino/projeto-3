...
describe('Validate Check-in Use Case', () => {
  beforeEach(async () => {
    ...
    sut = new ValidateCheckInUseCase(checkInRepository) // sut => system under test
  ...
  it('should not be able to validate the check-in after 20 minutes of its creation', async () => {
    ...
    const TwwntyOneMinutesInMiliseconds = 21 * 60 * 1000

    vi.advanceTimersByTime(TwwntyOneMinutesInMiliseconds)
    await expect(() =>
      sut.execute({
        checkInId: createdCheckIn.id,
      }),
